...
@OnlyIn(Dist.CLIENT)
public class ResourceMinerScreen extends AbstractContainerScreen<ResourceMinerContainer> {
    ...
    public ResourceMinerScreen(ResourceMinerContainer menu, Inventory playerInventory, Component tile) {
    ...
    private void renderTooltips(PoseStack poseStack, int mouseX, int mouseY) {
        ...
        if (actualX >= 43 && actualX <= 133 && actualY >= 26 && actualY <= 30) {
            Component text = Component.nullToEmpty(GuiFormatter.formatProgress(menu.getBrokenBlocksProgress()));
            renderTooltip(poseStack, text, mouseX, mouseY);
        }
    ...
    private void drawBlocksBroken(PoseStack poseStack) {
        String text = menu.getBlocksBroken() + " / " + menu.getScannedBlocks();
        ...
        fill(
                ...
                i + 44,
                j + 27,
                i + 44 + GuiUtil.lerp(0, 89, menu.getBrokenBlocksProgress()),
                j + 29,
                0xFFFFFFFF
        );
    }
